...
class SoundEngine {
  ...
  constructor() {
    // Muted by default until user interacts
  }
  ...
  public playVictoryFanfare() {
    ...
    notes.forEach((n) => {
      ...
      const gain = this.ctx!.createGain();
      osc.type = 'triangle';
      osc.frequency.setValueAtTime(n.f, now + timeAcc);
      gain.gain.setValueAtTime(0.25, now + timeAcc);
      gain.gain.exponentialRampToValueAtTime(0.01, now + timeAcc + n.d);

      osc.connect(gain);
      gain.connect(this.ctx!.destination);
      osc.start(now + timeAcc);
      osc.stop(now + timeAcc + n.d);
      timeAcc += n.d * 0.9;
    });
  }
  ...
  public playElectricZap() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const gain = this.ctx.createGain();
      osc.type = 'sawtooth';
      osc.frequency.setValueAtTime(200 + Math.random() * 800, now + i * 0.04);
      osc.frequency.linearRampToValueAtTime(100 + Math.random() * 300, now + i * 0.04 + 0.04);
  ...
  public playAlarm() {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const gain = this.ctx.createGain();
      osc.type = 'sawtooth';
      osc.frequency.setValueAtTime(700, now + i * 0.2);
      osc.frequency.setValueAtTime(950, now + i * 0.2 + 0.1);
